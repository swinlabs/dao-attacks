...
module.exports = function(deployer, network, accounts) 
{
   ...
   deployer.deploy(HoneyPotData, {from: dataAddr}).then((honeyPotData) => {
      ...
      return deployer.deploy(HoneyPotLogic, honeyPotData.address, { from: owner }).then((honeyPotLogic) =>{
         ...
         return deployer.deploy(HoneyPotAttack, honeyPotLogic.address, { from: attacker }).then((honeyPotAttack) =>{
            ...
            fileSystem.writeFileSync('honeyPotData.address.json', JSON.stringify({ 'honeyPotData': honeyPotAttack.address }));
