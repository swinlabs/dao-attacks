...
contract('Re-entrancy test', async (accounts) => 
{
    ...
    describe('get() test', () => 
    {
        it('return value at memory address 0x40 and 0x60', async () => {
            //check memory address
            console.log(`BEFORE CALLING THE FALLBACK`);
            var _memorySize = await _proxy.getMsize();
            console.log(`memory size is: ${_memorySize}`);
            var _memoryAt0x40 = await _proxy.getMemoryAt0x40();
            console.log(`default value at memory address 0x40: ${_memoryAt0x40}`);
            var _memoryAt0x60 = await _proxy.getMemoryAt0x60();
            console.log(`default value at memory address 0x60: ${_memoryAt0x60}`);
            var _memoryAt0x80 = await _proxy.getMemoryAt0x80();
            console.log(`default value at memory address 0x80: ${_memoryAt0x80}`);
        });
        it('attacker uses the re-entrancy bug to drain funds', async () => 
        {
            ...
            //check memory address
            console.log(`AFTER CALLING THE FALLBACK`);
            var _memorySize = await _proxy.getMsize();
            console.log(`memory size is: ${_memorySize}`);
            var _memoryAt0x40 = await _proxy.getMemoryAt0x40();
            console.log(`default value at memory address 0x40: ${_memoryAt0x40}`);
            ...
            var _memoryAt0x80 = await _proxy.getMemoryAt0x80();
            console.log(`default value at memory address 0x80: ${_memoryAt0x80}`);
            var _memoryAt0x80After = await _proxy.getMemoryAt0x80After();
            console.log(`default value at memory address 0x80 after: ${_memoryAt0x80After}`);
            await honeyPotProxy.put(
                { 
                    from: user2, 
                    value: web3.utils.toBN(ether).mul(web3.utils.toBN(50)) 
