    ...
    describe('get() test', () => 
    {
        it('attacker uses the re-entrancy bug to drain funds', async () => 
        {
            ...
            await HoneyPotContract.put(
                { 
                    from: attacker, 
                    value: web3.utils.toBN(ether).mul(web3.utils.toBN(_testAmount)) 
                });
            // var _balanceOfAttacker = await HoneyPotContract.getBalanceOf(attacker);
            // var _attackerBalance = await web3.eth.getBalance(attacker);
            // console.log(`balance of attacker on HoneyPot Contract is ${_balanceOfAttacker / ether}`);
            // console.log(`total balance of attacker is ${_attackerBalance / ether}`);
            // console.log(`start get attacker's Ether out of HoneyPot contract`);
            // var _getAmount = web3.utils.toBN(ether).mul(web3.utils.toBN(_testAmount / 2));
            // try {
            //     var _result = await HoneyPotContract.get(_getAmount, {
            //         from: attacker
            //     });
            //     console.log(`result of get Ethers ${_result}`);
            // } catch (err) {
            //     console.log(`error with get function: ${err}`);
            // }
            
            // var __balanceOfAttacker = await HoneyPotContract.getBalanceOf(attacker);
            // var __attackerBalance = await web3.eth.getBalance(attacker);
            // console.log(`balance of attacker on HoneyPot Contract after test is ${__balanceOfAttacker / ether}`);
            // console.log(`total balance of attacker after test is ${__attackerBalance / ether}`);
            await HoneyPotContract.put(
                { 
                    from: user1, 
            ...
            let maxIterations = 5;
            let triggerAmount = web3.utils.toBN(0.1 * ether);
            let leftoverAmount = web3.utils.toBN(0.01 * ether);
